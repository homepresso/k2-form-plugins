/**
 * QR Code Control for K2 SmartForms
 * Generates and displays QR codes using qrcode-generator library
 */
(function() {
  'use strict';
  ...
  // QR Code Generator - Embedded lightweight library
  ...
  var qrcode = (function() {
    var qrcode = function(typeNumber, errorCorrectionLevel) {
      ...
      _this.createSvgTag = function(cellSize, margin, alt, title) {
        ...
        qrSvg += (title.text || alt.text) ? ' role="img"' : '';
        qrSvg += title.id ? ' aria-labelledby="' + title.id + '"' : '';
        qrSvg += alt.id ? ' aria-describedby="' + alt.id + '"' : '';
        qrSvg += '>';
        qrSvg += title.text ? '<title id="' + title.id + '">' + title.text + '</title>' : '';
        qrSvg += alt.text ? '<description id="' + alt.id + '">' + alt.text + '</description>' : '';
        qrSvg += '<rect width="100%" height="100%" fill="' + (opts.bg || '#ffffff') + '" cx="0" cy="0"/>';
        qrSvg += '<path d="';
        ...
        qrSvg += '" stroke="transparent" fill="' + (opts.fg || '#000000') + '"/>';
        qrSvg += '</svg>';
  ...
  if (!window.customElements.get('qr-code')) {
    window.customElements.define('qr-code', class QRCodeControl extends HTMLElement {
      constructor() {
        super();
        this._hasRendered = false;

        // Properties with defaults
        this._value = 'https://example.com';
        this._size = 150;
      ...
      _render() {
        ...
        this._container.className = 'qr-container';
      ...
      _generateQRCode() {
        ...
        if (!this._value) {
          this._container.innerHTML = '<div style="color:#999;font-size:12px;">No data</div>';
          return;
        }

        try {
          // Create QR code (type 0 = auto-detect size)
          var qr = qrcode(0, this._errorCorrectionLevel);
          qr.addData(this._value);
          qr.make();

          // Generate SVG
          var cellSize = Math.floor(this._size / (qr.getModuleCount() + this._quietZone * 2));
          cellSize = Math.max(cellSize, 1);

          var svg = qr.createSvgTag({
            cellSize: cellSize,
            margin: this._quietZone * cellSize,
            fg: this._foregroundColor,
            bg: this._backgroundColor,
            scalable: true
          });

          // Create wrapper and set size
          ...
          this._container.innerHTML = '<div style="color:#d32f2f;font-size:12px;">Error generating QR code</div>';
        }
      }
      ...
      set Value(v) {
        this._value = v || '';
        if (this._hasRendered) this._generateQRCode();
        safeRaisePropertyChanged(this, 'Value');
      }

      get size() { return this._size; }
      set size(v) {
        this._size = parseInt(v) || 150;
        if (this._hasRendered) this._generateQRCode();
      ...
      set foregroundColor(v) {
        this._foregroundColor = v || '#000000';
        if (this._hasRendered) this._generateQRCode();
        safeRaisePropertyChanged(this, 'foregroundColor');
      }
      get ForegroundColor() { return this.foregroundColor; }
      set ForegroundColor(v) { this.foregroundColor = v; }
      ...
      get errorCorrectionLevel() { return this._errorCorrectionLevel; }
      set errorCorrectionLevel(v) {
        var valid = ['L', 'M', 'Q', 'H'];
        this._errorCorrectionLevel = valid.includes(v) ? v : 'M';
        if (this._hasRendered) this._generateQRCode();
        safeRaisePropertyChanged(this, 'errorCorrectionLevel');
      }
      get ErrorCorrectionLevel() { return this.errorCorrectionLevel; }
      set ErrorCorrectionLevel(v) { this.errorCorrectionLevel = v; }

      get quietZone() { return this._quietZone; }
      set quietZone(v) {
        this._quietZone = parseInt(v) || 4;
        if (this._hasRendered) this._generateQRCode();
        safeRaisePropertyChanged(this, 'quietZone');
      }
      get QuietZone() { return this.quietZone; }
      set QuietZone(v) { this.quietZone = v; }
