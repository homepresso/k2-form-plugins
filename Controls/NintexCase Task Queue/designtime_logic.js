if (!window.__nintexcasetaskqueueDesigntimeLoaded) {
  window.__nintexcasetaskqueueDesigntimeLoaded = true;

/**
 * NintexCase Task Queue Control - Design Time Logic
 */
(function() {
  'use strict';

  if (!window.customElements.get('nintexcase-task-queue')) {
    window.customElements.define('nintexcase-task-queue', class extends HTMLElement {
      ...
      _buildQueue() {
        ...
        this._sampleItems.forEach((item, index) => {
          ...
          assignedByCell.textContent = item.AssignedBy;
          ...
          caseCell.textContent = item.Case;
